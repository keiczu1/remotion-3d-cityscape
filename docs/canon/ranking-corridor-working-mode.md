...
Если пользователь сам хочет свой вариант или явно подтверждает выход за пределы канонической базы, ИИ может работать с `user-custom` или `ai-custom` и потом зафиксировать это в `launch-card`.
