...
2. элемент доходит до полного ролика;
3. проект получает финальное утверждение;
4. после отдельного ручного решения элемент либо остается локальным, либо переносится в переиспользуемый слой.
