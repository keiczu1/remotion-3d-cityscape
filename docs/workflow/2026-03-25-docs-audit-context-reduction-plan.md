...
- выполнено: `ground` уточнен как theme-driven слой поверхности: park / asphalt / sand / desert / grid и другие режимы допустимы по усмотрению режиссерской логики сцены.
- выполнено: примеры для `ground` смягчены до inspirational-ориентиров; канон больше не звучит как фиксированный список допустимых приемов.
