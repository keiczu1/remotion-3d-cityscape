...
4. Получить один пакет вопросов.
5. После ответа получить короткую `launch-card`.
6. Дойти до `preview-gate`, а не сразу до полного ролика.
7. После review построить полную композицию без обязательного экспорта финального медиафайла.
8. После финального статуса провести аудит библиотеки и обновить документацию.
...
4. `ranking-corridor-launch` классифицирует тему и задает один пакет вопросов.
5. После ответа собирается короткая `launch-card`.
6. `ranking-corridor-production` поднимает project-container и рабочие артефакты.
7. Собираются данные, локальные ассеты и `asset-manifest`.
8. Делается `preview-gate`.
9. После решения по предпросмотру допускается полная сборка композиции.
10. После полной сборки композиции фиксируется финальный статус проекта.
11. Только после финального статуса запускается аудит библиотеки.
12. При необходимости обновляются registry, reusable-слой и связанные docs.
