...
import { getEntranceEffect } from "../entrance-effects";
...
export const getBiographyOverlayStateForIndex = ({
    ...
    const triggerMilestone = index > 2 ? milestones[index - 3] : null;
    const appearanceFrame = index <= 3 ? -100 : (triggerMilestone ? triggerMilestone.arriveFrame - 20 : Math.max(0, milestone.arriveFrame - 150));
    const revealProgress = smoothstep(appearanceFrame, appearanceFrame + 35, frame);
...
const getOverlayCandidateIndices = ({
    ...
    const rangeEnd = Math.min(totalCount - 1, focusedIndex + 5);
...
export const FocusedBiographyOverlay = ({
    ...
    const overlayStates = allOverlayStates.filter((state): state is NonNullable<typeof state> => {
            ...
            return isVisibleInViewport || state.revealProgress > 0.001;
        });
    ...
    return (
        <AbsoluteFill style={{ pointerEvents: "none", overflow: "visible" }}>
            {overlayStates.map(({ index, item, milestone, pedestalHeight, presentationState, overlayProgress, revealProgress, appearanceFrame }) => {
                ...
                // ── entrance effect ──
                const entranceAnimFrame = Math.max(0, frame - appearanceFrame);
                    ...
                    >
                        ...
                        >
                            <PortraitBiographySteleHero
                                ...
                                delay={Math.max(0, appearanceFrame)}
                                frame={heroFrame}
                                fps={fps}
                                showPedestal={false}
                                showFlagMast={false}
                                showInfoSideCard={false}
