...
const getMediaLayout = (aspectRatio: number): MediaLayout => {
    ...
    const imageWidth = Math.min(imageHeight * safeAspectRatio, maxImageWidth);
...
const PowerMetric = ({
    ...
    const powerLabelFontSize = 1.0 + scaleFactor * 0.34;
    return (
    <group position={[0, VISITS_VALUE_Y, 0.3]}>
        ...
        <Text position={[0, -2.2, 0]} color="#475569" fontSize={powerLabelFontSize} anchorX="center" anchorY="middle" fontWeight="bold" fillOpacity={fillOpacity === undefined ? 0.72 : fillOpacity * 0.72} letterSpacing={0.03} material-toneMapped={false}>
            POWER
        </Text>
    </group>
    );
...
const FullCard = memo(({
    ...
    <group position={[0,0,0.5]}>
        ...
        <PowerMetric value={item.base_stat_total.toString()} numericValue={item.base_stat_total} fillOpacity={opacity} scaleFactor={item.relHeight} />
...
export const HeroPedestal = ({
    ...
    const nameFontSize = Math.min(
        formattedName.lineCount > 1 ? 1.6 : 2.0, // Увеличенные лимиты для одной/двух строк
        (SCREEN_WIDTH - 1.1) / (Math.max(4, formattedName.longestLineLength) * 0.5), // Больший скейл
    );
    const isCinematic = frame > sequenceCompleteFrame;
    const typeLabel = item.types.join(" / ").toUpperCase();
    ...
    }), [dashboardBaseY, mediaLayout.cardBottomLocalY, mediaLayout.cardTopLocalY, worldX, worldZ]);
    ...
    const cardRevealRenderers: CardRevealRenderers = {
        ...
        renderRankText: () => null,
        renderMedia: ({ opacity = 1, zPos = POKEMON_IMAGE_Z } = {}) => (
            <PokemonImage
                texture={texture}
                yPos={mediaLayout.imageY}
                zPos={zPos}
        ...
        renderVisits: ({ text, fillOpacity, counterProgress: visitsCounterProgress } = {}) => (
            <PowerMetric
                value={text ?? item.base_stat_total.toString()}
                numericValue={item.base_stat_total}
    ...
    return (
        <group position={[0, liveDashboardMetrics.rootWorld[1], 0]}>
            <Effect
                ...
                visitsText={item.base_stat_total.toString()}
