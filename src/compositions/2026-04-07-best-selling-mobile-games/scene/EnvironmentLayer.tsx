...
export const EnvironmentLayer = () => {
    ...
    const skyR = interpolate(env.totalProgress, [0, 0.25, 0.5, 0.75, 1], [0.55, 0.4, 0.15, 0.05, 0.02]);
    const skyG = interpolate(env.totalProgress, [0, 0.25, 0.5, 0.75, 1], [0.85, 0.5, 0.2, 0.08, 0.02]);
    const skyB = interpolate(env.totalProgress, [0, 0.25, 0.5, 0.75, 1], [1.0, 0.65, 0.3, 0.15, 0.05]);
    ...
    const mainLightColor = new THREE.Color(
        interpolate(env.totalProgress, [0, 0.25, 0.5, 0.75, 1], [1.0, 0.9, 0.8, 0.7, 0.7]),
        interpolate(env.totalProgress, [0, 0.25, 0.5, 0.75, 1], [1.0, 0.9, 0.8, 0.7, 0.7]),
        interpolate(env.totalProgress, [0, 0.25, 0.5, 0.75, 1], [0.9, 1.0, 1.0, 1.0, 1.0]),
    );
    const ambientIntensity = interpolate(env.totalProgress, [0, 0.25, 0.5, 0.75, 1], [0.8, 0.6, 0.3, 0.1, 0.1]);
    const forestOpacity = interpolate(env.totalProgress, [0.35, 0.5], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });

    const cloudColor = new THREE.Color(
        interpolate(env.totalProgress, [0, 0.25, 0.75, 1], [1.0, 0.6, 0.1, 0.05]),
        interpolate(env.totalProgress, [0, 0.25, 0.75, 1], [1.0, 0.65, 0.12, 0.05]),
        interpolate(env.totalProgress, [0, 0.25, 0.75, 1], [1.0, 0.7, 0.15, 0.05]),
    );
    ...
    const showHighway = true; // Видно с самого начала
    ...
    return (
        <>
            ...
            <LocalCorridorReliefGround maxX={maxX} groundY={GROUND_Y} />
