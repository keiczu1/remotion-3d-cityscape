...
export const formatRevenue = (val: number) => {
    if (val >= 1000) {
        return `$${(val / 1000).toFixed(1).replace(/\.0$/, "")}B`;
    }
    return `$${Math.round(val)}M`;
};

const rollCounterValue = (target: number, progress: number) => {
    return formatRevenue(Math.floor(target * progress));
};
...
const FullCard = memo(({
    ...
    <group position={[0,0,0.5]}>
        ...
        <PowerMetric value={formatRevenue(item.base_stat_total)} numericValue={item.base_stat_total} fillOpacity={opacity} scaleFactor={item.relHeight} />
...
export const HeroPedestal = ({
    ...
    const rawTexture = useTexture(staticFile(`/ranking-corridor/2026-04-07-best-selling-mobile-games/images/${item.image_file}`));
    ...
    const typeLabel = [...item.types].pop()?.toUpperCase() || "";
    ...
    const cardRevealRenderers: CardRevealRenderers = {
        ...
        renderVisits: ({ text, fillOpacity, counterProgress: visitsCounterProgress } = {}) => (
            <PowerMetric
                value={text ?? formatRevenue(item.base_stat_total)}
    ...
    return (
        <group position={[0, liveDashboardMetrics.rootWorld[1], 0]}>
            <Effect
                ...
                visitsText={formatRevenue(item.base_stat_total)}
