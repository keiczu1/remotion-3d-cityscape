...
const getVipOrbitCameraState = (profile: VipFocusProfile, frame: number): CameraState => {
    ...
    if (isFinalOrbitHold) {
        ...
        const framingZOffset = Math.max(16, 28 - pushIn * 10 - mix(2.4, 4.8, profile.heightNorm));
    ...
    const framingZOffset = Math.max(18, 28 - pushIn * 10 - mix(1.2, 3.8, profile.heightNorm));
