...
import {
  ...
  DynamicInstances,
} from "../../../lib/ranking-corridor/three";
...
export const localGetCorridorReliefHeight = ({
  ...
  const rocks =
    (Math.sin(worldX * 0.03) + Math.sin(worldX * 0.07 + worldZ * 0.05)) *
    25 *
    (Math.sin(worldZ * 0.04) + 1);
  ...
  if (worldX < maxX * 0.5) {
    let flattenFactor = 0;
    if (worldX > maxX * 0.4) {
      flattenFactor = interpolate(worldX, [maxX * 0.4, maxX * 0.5], [0, 1], {
  ...
  ({
    ...
    const terrainGeometry = useMemo(() => {
      ...
      for (let index = 0; index < positions.count; index++) {
        ...
        const worldY = localGetCorridorReliefHeight({
          worldX,
          worldZ,
          maxX,
          laneCenterZ,
    ...
    const rockMatrices = useMemo(() => {
      ...
      for (let index = 0; index < 400; index++) {
        const worldX = seededRange(
          `corridor-relief-rock-world-x-${index}`,
          -500,
          maxX + 500,
        );
        const worldZ =
          seededSign(`corridor-relief-rock-side-${index}`) *
          seededRange(`corridor-relief-rock-z-${index}`, 150, 1350);
        const progress = Math.min(1, Math.max(0, worldX / Math.max(1, maxX)));
        const scale =
          seededRange(`corridor-relief-rock-scale-${index}`, 2, 7) *
          (1 + progress * 1.5);
        const terrainY = localGetCorridorReliefHeight({
          worldX,
          worldZ,
          maxX,
          laneCenterZ,
    ...
    const puddleMatrices = useMemo(() => {
      ...
      for (let index = 0; index < 150; index++) {
        const worldX = seededRange(
          `corridor-relief-puddle-world-x-${index}`,
          maxX * 0.3,
          maxX * 1.1,
        );
        const laneOffset = seededRange(
          `corridor-relief-puddle-offset-${index}`,
          34,
          88,
        );
        const worldZ =
          laneCenterZ +
          seededSign(`corridor-relief-puddle-side-${index}`) * laneOffset;
        const radius = seededRange(
          `corridor-relief-puddle-radius-${index}`,
          5,
          25,
        );
        const terrainY = localGetCorridorReliefHeight({
