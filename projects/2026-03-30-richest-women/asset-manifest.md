...
- `2026-04-02 rail handoff correction`: user-review подтвердил, что hard cut между башнями ломает формат. Main-pass handoff исправлен на короткий rail-проезд: текущая башня сохраняет focus до `arriveFrame` следующей, а камера за `36` кадров физически доезжает по `X/lookX/Z` к новому пьедесталу, после чего продолжает читаемый hold и orbit
