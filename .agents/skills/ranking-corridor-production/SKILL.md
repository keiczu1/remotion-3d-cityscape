...
- для такого implementation-locked пакета с `timingContract: adaptive` в `Reuse without changes` явно перечисляй motion-locked часть: camera path math, базовую scene progression, intro/main handoff и VIP/tower orbit behavior, не выдавая count-aware timing за полностью locked слой;
- для такого implementation-locked пакета с `timingContract: adaptive` в `Allowed adaptation` оставляй только data normalization, рабочие offsets, безопасную дистанцию камеры, topic-specific framing и count-aware retiming через закрепленный `timingPolicyId`;
- для adaptive camera preset дополнительно фиксируй в `camera-preview` поля `Object count`, `Target duration band`, `Timing policy` и `Finale tail policy`; текущие adaptive preset трактуются как `60fps-only`;
- после обновления `build-plan.md` валидатор должен подтвердить, что `camera-preview` согласован с выбранным `scenePresetPackage` из `launch-card.md`, exact `sourceOfTruthFiles` из registry, launch-card полем `что считается зафиксированным без пересборки`, registry `supportedCountRange` и `targetDurationBandSeconds`; `supportedFps` считай contract-level metadata preset-а, а actual count auto-check — гарантированным для snapshot-проектов с `public/ranking-corridor/<project-slug>/data.json`;
